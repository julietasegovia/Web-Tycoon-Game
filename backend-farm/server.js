...
app.post('/api/auth/signup', async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ user: { id: user.id, username: user.username, email: user.email, profilePicture: user.profilePicture }, token })
...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    ...
    res.json({ user: { id: user.id, username: user.username, email: user.email, profilePicture: user.profilePicture }, token })
...
app.get('/api/game/state', requireAuth, async (req, res) => {
  try {
    ...
    res.json({ state: save ? JSON.parse(save.data) : null })
  } catch (err) {
...
// Frontend resizes/compresses the image before sending it, so this just
// validates shape + a sane size ceiling before writing it to the DB.

app.put('/api/auth/profile-picture', requireAuth, async (req, res) => {
  const { profilePicture } = req.body

  if (typeof profilePicture !== 'string' || !profilePicture.startsWith('data:image/')) {
    return res.status(400).json({ message: 'profilePicture must be a base64 image data URL' })
  }
  ...
  try {
    ...
    console.error('PUT /api/auth/profile-picture error:', err.message)
